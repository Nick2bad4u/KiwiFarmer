...
def test_get_url_list_db_error(mock_mysql_connector, caplog):
    mock_mysql_connector[0].connect.side_effect = mysql.connector.Error("Connection failed")
    urls = get_url_list(page=1)
    assert urls == []
...
def test_process_reaction_page(MockReaction, MockReactionPage, tmpdir, mock_mysql_connector):
    ...
    mock_reaction_page.get_reaction_soups.return_value = [BeautifulSoup("<div>Reaction</div>", 'lxml')]
