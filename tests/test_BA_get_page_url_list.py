...
def test_generate_page_urls(mock_functions, mock_os_listdir, mock_open_file, mock_beautifulsoup, tmpdir):
    ...
    generate_page_urls.generate_page_urls(
        str(input_dir), str(output_dir), page_list_filename)
    ...
    with open(output_file_path, 'r') as f:
        ...
        ) == 'https://kiwifarms.st/threads/thread1/page-1/'
        assert lines[1].strip(
        ) == 'https://kiwifarms.st/threads/thread1/page-2/'
        assert lines[2].strip(
        ) == 'https://kiwifarms.st/threads/thread1/page-3/'
        assert lines[3].strip(
        ) == 'https://kiwifarms.st/threads/thread2/page-1/'
        assert lines[4].strip(
        ) == 'https://kiwifarms.st/threads/thread2/page-2/'
        assert lines[5].strip(
        ) == 'https://kiwifarms.st/threads/thread2/page-3/'
