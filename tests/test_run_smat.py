...
def test_ingest(mock_process_thread, mock_get_threads, mock_elasticsearch, mock_redis):
    ...
    mock_redis.assert_called_once_with(host=run_smat.REDIS_HOST, port=run_smat.REDIS_PORT, db=run_smat.REDIS_DB)
    ...
    mock_process_thread.assert_any_call(thread_unique='thread1', last_mod='last_mod1', rdb=mock_rdb, es=mock_es)
    mock_process_thread.assert_any_call(thread_unique='thread2', last_mod='last_mod2', rdb=mock_rdb, es=mock_es)
