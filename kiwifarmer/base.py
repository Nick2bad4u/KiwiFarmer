...
class Page:
    ...
    def extract_thread_id(self):
        ...
        thread_id_element = self.thread_page.find(
    ...
    def extract_thread_title(self):
        ...
        thread_title_element = self.thread_page.find(
    ...
    def get_post_soups(self):
        ...
        """
        _post_soups = self.thread_page.find_all(
            'div', {'class': "message-inner"})
        post_soups = [post_soup for post_soup in _post_soups if post_soup.find(
...
class Post:
    ...
    def __init__(self, post):
        ...
        blockquotes_text, blockquotes_sources = functions.get_post_blockquotes(
            message=message)
        ...
        self.post_author_username = functions.get_post_author_username(
            post=self.post)
        # Store post author user ID as class variable
        self.post_author_user_id = functions.get_post_author_user_id(
            post=self.post)
        ...
        self.blockquote_insertions = self.generate_blockquote_insertions(
            blockquotes_text, blockquotes_sources)

        # Generate list of dicts for link fields
        self.link_insertions = self.generate_link_insertions(
            links, links_texts)
...
class Reaction:
    ...
    def __init__(self,
        ...
        self.reaction_author_username = functions.get_reaction_author_username(
            reaction=self.reaction)
        # store reaction author user ID as class variable
        self.reaction_author_user_id = functions.get_reaction_author_user_id(
            reaction=self.reaction)
        ...
        self.reaction_name = functions.get_reaction_name(
            reaction=self.reaction)
        # store reaction author timestamp as class variable
        self.reaction_timestamp = functions.get_reaction_timestamp(
            reaction=self.reaction)
...
class User:
    ...
    def __init__(self, user_page):
        ...
        self.user_username = functions.get_user_username(
        ...
        self.user_messages = functions.get_user_messages(
            user_page=self.user_page)
        # store user reaction score as class variable
        self.user_reaction_score = functions.get_user_reaction_score(
            user_page=self.user_page)
        ...
        self.user_joined, self.user_last_seen = functions.get_user_timestamps(
            user_page=self.user_page)
...
class Following:
    ...
    def __init__(self,
        ...
        # store user username as class variable
        self.user_id = functions.get_following_user_id(
